...
- [x] **Task 7 — La verifica** (AC: 1, 2, 3, 4, 5, 6)
  ...
  - [x] Apro un cliente, apro una scheda nuova (o uso una già aperta), e dalla riga della card **arrivo alla schermata di compilazione** (Task 5).
  - [x] Vedo i blocchi uno sotto l'altro, nell'ordine dell'intervista, ciascuno con le sue domande, e scorro dall'alto in basso senza che niente mi fermi (AC1).
  - [x] Sopra ogni campo c'è la domanda, sotto c'è l'aiuto, e le **due domande senza aiuto** non lasciano nessun buco (AC2).
  - [x] Scrivo in un campo lungo un testo di dieci righe: **il campo cresce** e non compare nessuna barra di scorrimento interna; a campo vuoto è alto tre righe (AC3).
  - [x] Entro nel campo: bordo e alone sono nella tinta della prequalifica, e non compare l'anello predefinito del browser (AC3).
  - [x] La domanda di tipo **scelta singola** offre le sue quattro opzioni più `Nessuna risposta`, e si può lasciare su `Nessuna risposta` (AC4).
  - [x] Il campo di tipo **testo breve** è una riga sola, e ci si scrive dentro.
  ...
  - [x] Scorro fino in fondo senza scrivere niente: nessun asterisco, nessun avviso, nessun campo che si colora, nessun messaggio (AC5).
  - [x] Ricarico la pagina: **quello che avevo scritto non c'è più**, ed è quello che questa story dichiara. Se ci fosse, qualcosa scrive senza che nessun task lo abbia chiesto.
  - [x] Da tastiera: Tab attraversa i campi nell'ordine visivo, il fuoco si vede su ognuno, il ritorno indietro è raggiungibile.
  - [x] Larghezza 375px: nessuno scorrimento laterale, i campi restano da almeno 44px, le domande lunghe vanno a capo, il titolo del blocco più lungo — «Solo sopra le 50 persone, o quando l'interlocutore non è il titolare», 67 caratteri — non trabocca (AC6, NFR4).
  - [x] Un indirizzo cucito a mano con la scheda di un cliente sotto **un altro** cliente risponde 404 e non una pagina che mostra il contesto sbagliato.
  - [x] Nessun errore in console e nessun avviso di idratazione.

  Aggiunte dalla revisione del 9 agosto 2026, e sono la verifica dei patch applicati:
  - [x] In cima alla schermata leggo **«Qui ancora non si salva: quello che scrivi resta finché non ricarichi la pagina.»**, in tinta d'avviso e non d'errore, e non mi impedisce niente. Non compare quando la scheda non si carica, dove il messaggio giusto è già un altro.
  - [x] Nella card delle schede il **collegamento sulla data** si riconosce come tale — si sottolinea al passaggio, e il fuoco da tastiera si vede — ed è alto almeno 44px. Da guardare anche l'effetto sull'altezza delle righe: la voce ora è più alta di prima, e se in elenco stona è qui che si vede.
  - [x] Il campo lungo cresce ancora senza barra di scorrimento interna, adesso che porta `overflow: auto` invece di `hidden`: se una barra comparisse a campo pieno, la copia invisibile e il campo non hanno più la stessa scatola, ed è esattamente l'informazione che `hidden` nascondeva.
  - [x] Un aiuto lungo o con un token senza spazi va a capo dentro la card e non fa scorrere la pagina di lato a 375px.
...
### Verifica di sessione, 9 agosto 2026

Diciotto caselle su diciotto, fatte da Luca sulla pagina vera dopo i patch di revisione: le quattordici del Task 7 e le quattro aggiunte dalla revisione. **Story done.**

**Il tipo `numero` è stato provato, ed è l'unico dei quattro controlli che i dati veri non esercitano.** Aggiunta una domanda di tipo numero dal `/questionario`, aperta una scheda nuova su un cliente di prova, guardato che il campo accetti quello che si scrive, poi **la domanda è stata tolta del tutto** invece che lasciata disattivata come il task diceva. Va scritto, perché il database non porta più nessuna traccia della prova: **zero domande di tipo `numero`, zero domande disattivate**, e chi ricontasse leggerebbe uno stato identico a quello di partenza e ne dedurrebbe che la casella è stata spuntata dicendo il falso. È la stessa ragione per cui la 3.1 ha dichiarato le schede di prova rimosse dopo la verifica.

**Lo stato del database dopo la verifica, contato il 9 agosto 2026:** 3 clienti, **0 schede, 0 risposte**, 9 blocchi, **24 domande tutte attive**, quindici migrazioni a registro. Identico alla partenza: le schede di prova sono state tolte, come dopo la 3.1.

Ha una conseguenza che vale la pena registrare, perché tocca una voce rimandata da poche ore: `answers` è **ancora vuota**, quindi la finestra in cui `block_id` si aggiungerebbe `not null` senza riempimento — la stessa che ha reso gratuita la 0015 — **non si è chiusa**. Si chiuderà alla prima scheda che resta, cioè con la Story 3.3, che è la prima in cui compilare una scheda vera ha senso. Chi riprendesse quella voce dopo la 3.3 troverà un costo diverso da quello scritto lì.
...
| 9 agosto 2026 | 1.2 | Verifica di sessione fatta: diciotto caselle su diciotto, comprese le quattro aggiunte dalla revisione. Il tipo `numero` provato aggiungendo una domanda e togliendola dopo, quindi il database non ne porta traccia ed è dichiarato. Database identico alla partenza — 3 clienti, 0 schede, 0 risposte, 24 domande attive — perciò la finestra gratuita di `block_id` resta aperta fino alla 3.3. Status → done. | Luca |
