...
**Chiusa per le pillole di verdetto il 10 agosto 2026, con la Story 3.5**, nella forma che la voce prescriveva: tre inchiostri nuovi — `--ok-ink`, `--warn-ink`, `--bad-ink` — ~~usati **solo** dentro le pillole~~, mentre i quattro pieni di questa decisione non si toccano. **Il «solo» è caduto il 10 agosto 2026, con la code review della Story 3.6:** `--bad-ink` colora anche la riga d'errore dell'intestazione di pagina, che è l'unico `.field__error` del software fuori da una card e quindi l'unico su `--bg`, dove `--bad` sta a 4,13:1 e non passa. Non è un'eccezione strappata alla decisione: è il ruolo che il nome del token diceva già — l'inchiostro per un fondo chiaro dove il pieno non arriva — applicato al secondo posto che ne ha avuto bisogno. I quattro pieni continuano a non toccarsi, che è la parte di questa riga che non è mai cambiata. La riga resta scritta e barrata perché due cose che diceva vanno corrette e non cancellate.
